...
* Subprocedure \mathcal{C}:
  ...
  \end{verbatim}
\begin{align*}
  C =& \left( \bigwedge_{s \in S}
    s = M_0(s) + \sum_{t \in ^\bullet s} t - \sum_{t \in s^\bullet} t
  \right) \land \\
...
\end{verbatim}
\begin{align*}
  C_\theta =& \left( \bigwedge_{s \in S} \left( s \Rightarrow
        \bigwedge_{t \in s^\bullet} \bigvee_{p \in t^\bullet} p
    \right) \right) \land \\
...
\end{verbatim}
\begin{align*}
  & \delta = \left( \sum_{A_\theta(s)} s \ge 0 \right)\\
\end{align*}
