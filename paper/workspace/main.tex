...
\begin{tikzpicture}[node distance=2cm]
  \node[place, tokens=1, label=left:a] (a) {};
  \node[place, tokens=0, label=left:b, below of=a] (b) {};
  \node[place, tokens=0, label=right:nbit2, right of=b, xshift=-1cm, yshift=+1cm] (nbit2) {};
  \node[transition, label=left:t1, above of=a, yshift=-1cm] (t1) {}
    edge [pre, bend left, min distance=2cm, out=225, in=-45] (b)
    edge [post] (a);
  \node[transition, label=left:t2] (t2) at ($(a)!0.5!(b)$) {}
    edge [pre] (a)
    edge [pre] (nbit2)
    edge [post] (b)
    edge [post] (nbit2);
