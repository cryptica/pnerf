...
\begin{tikzpicture}[
  ...
  \node[action, below=of satc] (modelc)
    {$A:=\text{Model}(C \cup \{\neg P\})$ \\
     $\text{Methods}:=\text{RefinementMethods}$};
  \node[decision, below=of modelc] (methods) {$Methods \neq \emptyset$};
  \node[action, below=of methods] (ctheta)
    {$\text{Refinement} \leftarrow \text{Methods}$ \\
     $\text{Methods}:= \text{Methods} \setminus \{\text{Refinement}\}$ \\
     $C_{\theta}:=C_{\text{Refinement}}(N, A)$};
  ...
  -| ([xshift=-2cm] satctheta.west) |- (methods.west);
  \draw[edge] (modelctheta.south) -- ([yshift=-0.5cm] modelctheta.south)
